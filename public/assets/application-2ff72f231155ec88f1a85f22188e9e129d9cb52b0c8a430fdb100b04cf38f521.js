...
(function() {
  $(document).on('turbolinks:load', function() {
    var gs_handle, gs_slider;
    gs_handle = $("#gs-handle");
    gs_slider = $("#gs-slider").slider({
      min: 0,
      max: 25,
      value: $("#gs").val()
    }, {
      create: function() {
        return gs_handle.text($(this).slider('value'));
      },
      slide: function(event, ui) {
        gs_handle.text(ui.value);
        return $("#goals_scored").val(ui.value);
      }
    });
    $("#goals_scored").val($("#gs").val());
    $('#gs-counter').text($('#goals_scored').val());
    return $('#goals_scored').on('input', function() {
      return $('#gs-counter').text($('#goals_scored').val());
    });
  });
...
(function() {
  $(document).on('turbolinks:load', function() {
    ...
    $("#goalie_select").attr('data-placeholder', 'Select goalie(s)').chosen({
      allow_single_deselect: true,
      no_results_text: 'No results matched',
      width: '100%'
    });
    $("#teams").attr('data-placeholder', 'Select team(s)').chosen({
      allow_single_deselect: true,
      no_results_text: 'No results matched',
      width: '100%'
    });
    return $("#seasons").attr('data-placeholder', 'Select season(s)').chosen({
      allow_single_deselect: true,
      no_results_text: 'No results matched',
      width: '100%'
    });
...
(function() {
  $(document).on('turbolinks:load', function() {
    var player_game_breakdown_active_column, player_penalty_breakdown_active_column, profile_active_column;
    ...
    return $('#player-penalty-breakdown table tbody td:nth-child(' + player_penalty_breakdown_active_column + ')').addClass('active-tcol');
  });

}).call(this);
(function() {
  $(document).on('turbolinks:load', function() {
    return $('#start_date').on('input', function() {
...
(function() {
  $(document).on('turbolinks:load', function() {
    var active_column, age_slider, gp_handle, gp_slider;
    gp_handle = $("#gp-handle");
    gp_slider = $("#gp-slider").slider({
      min: 0,
      max: 100,
      value: $("#gp").val()
    }, {
      create: function() {
        return gp_handle.text($(this).slider('value'));
      },
      slide: function(event, ui) {
        gp_handle.text(ui.value);
        return $("#games_played").val(ui.value);
      }
    });
    $("#games_played").val($("#gp").val());
    age_slider = $('#age-range-slider').slider({
      range: true,
      min: 18,
      max: 50,
      values: [$('#low_age').val(), $('#high_age').val()],
      slide: function(event, ui) {
        $('#low_age_range').val(ui.values[0]);
        $('#high_age_range').val(ui.values[1]);
        $('#low-age-handle').text(ui.values[0]);
        return $('#high-age-handle').text(ui.values[1]);
      }
    });
    $("#low-age-handle").text($('#low_age').val());
    $("#high-age-handle").text($('#high_age').val());
    $("#low_age_range").val($('#low_age').val());
    $("#high_age_range").val($('#high_age').val());
    ...
    $("#skater_select").attr('data-placeholder', 'Select skater(s)').chosen({
      allow_single_deselect: true,
      no_results_text: 'No results matched',
      width: '100%'
    });
...
(function() {
  $(document).on('turbolinks:load', function() {
    var legend, show_legend;
    show_legend = $("#show-legend");
    legend = $("#legend");
    return show_legend.on('click', function() {
      legend.toggle();
      if (show_legend.text() === '[-] Hide Legend') {
        return show_legend.html('<span>[+] Show Legend</span>');
      } else {
        return show_legend.html('<span>[-] Hide Legend</span>');
      }
    });
  });
